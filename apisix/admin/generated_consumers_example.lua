...
local function check_conf(id, conf, need_id, schema, opts)
    ...
    if conf.plugins then
        local ok, err = plugins.check_schema(conf.plugins, core.schema.TYPE_CONSUMER)
        if not ok then
    ...
    if conf.group_id and not opts.skip_references_check then
        ...
        if not res then
            return nil, {error_msg = "failed to fetch consumer group info by consumer group id [" .. conf.group_id .. "]: "
                                  .. err}
        end
